...
- 공유 이유: "이걸 친구한테 DM으로 보낼 이유가 있나?"에 한 문장으로 답할 수 있어야 함 (예: 같이 투자하는 동료에게, 미국주식 시작한 친구에게). 답이 안 나오면 탈락
...
- 후킹 4공식 (성과 상위 금융 콘텐츠에서 검증됨 — 제목·앵글에 최소 1개, 조합하면 더 강력):
  ① 날짜/리스타트 프레임: "2026년 하반기에는", "지금 다시 시작한다면" — 시의성+당사자성
  ② 구체적 숫자: "월 $500 만들려면 얼마 필요한가" — 막연한 표현이면 그 후보는 탈락
  ③ 사회적 비교: "평균 직장인은", "조용한 부자들은" — 정체성 자극
  ④ 권위자 반전: "버핏이 안 산 이유", "월가는 반대로 갑니다" — 통념 뒤집기 (사실 기반만, 발언 날조 금지)
  ①+④ 조합형이 최고 성과 (예: "2026년에 버핏이 현금만 쌓는 이유")
- 3개 후보는 서로 다른 감정 트리거(손해회피/실행욕구/저장가치/공유욕구)로 다양하게
